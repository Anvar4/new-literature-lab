...
export const VirtualLabPage: React.FC = () => {
	...
	const handleTopicChange = (value: string) => {
		...
		setActiveType(newTopicId > 100 ? 'seminar' : 'maruza')
			...
			<div className='edu-container'>
				<div className='flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 animate-fade-in'>
					...
					<div className='flex flex-col gap-4 w-full md:w-auto'>
						<div className='flex p-1 bg-muted rounded-xl border border-border/50 w-full md:w-fit'>
							<Button
								variant={activeType === 'maruza' ? 'default' : 'ghost'}
								size='sm'
								onClick={() => handleTypeChange('maruza')}
								className={`flex-1 md:flex-none rounded-lg px-6 ${activeType === 'maruza' ? 'edu-gradient-bg text-white' : ''}`}
							>
								{t('lab.maruza')}
							</Button>
							<Button
								...
								className={`flex-1 md:flex-none rounded-lg px-6 ${activeType === 'seminar' ? 'edu-gradient-bg text-white' : ''}`}
							>
								{t('lab.seminar')}
							</Button>
						</div>

						<div className='flex items-center gap-4 bg-background p-2 rounded-xl border overflow-hidden w-full'>
							...
							>
								<SelectTrigger className='flex-1 md:w-[400px] font-bold border-none shadow-none focus:ring-0 overflow-hidden'>
									<SelectValue placeholder={t('lab.placeholder')} />
								</SelectTrigger>
								<SelectContent className='max-h-[400px] w-screen max-w-[95vw] md:w-[500px]'>
									{allTopics.map(topic => (
